...
if docker compose version >/dev/null 2>&1; then
  cid="$(docker compose -f "$ORACLE_CASE_DIR/docker-compose.yml" -p "$COMPOSE_PROJECT_NAME" ps -q struts2 | sed -n '1p' | tr -d '[:space:]')"
else
  cid="$(docker-compose -f "$ORACLE_CASE_DIR/docker-compose.yml" -p "$COMPOSE_PROJECT_NAME" ps -q struts2 | sed -n '1p' | tr -d '[:space:]')"
fi
[[ -n "$cid" ]] || { echo "Error: could not resolve Struts2 container" >&2; exit 1; }
