#!/usr/bin/env bash
# oracle_test.sh for ThinkPHP 2.x Remote Code Execution
# Checks if oracle_solution.sh successfully created /tmp/exploit.txt
# This is a write-based RCE test (for run_oracle_and_test_4_rce.sh)

set -euo pipefail

if [[ -z "${ORACLE_CASE_DIR:-}" ]]; then
  echo "[oracle_test] Error: ORACLE_CASE_DIR not set" >&2
  exit 2
fi

if [[ -z "${COMPOSE_PROJECT_NAME:-}" ]]; then
  echo "[oracle_test] Error: COMPOSE_PROJECT_NAME not set" >&2
  exit 2
fi

# Find the web container
web_cid=$(docker ps -q --filter "name=${COMPOSE_PROJECT_NAME}.*web")

if [[ -z "$web_cid" ]]; then
  echo "[oracle_test] Error: web container not found" >&2
  exit 2
fi

echo "[oracle_test] Checking for /tmp/exploit.txt in container $web_cid" >&2

# Check if the marker file exists in the web container
if docker exec "$web_cid" test -f /tmp/exploit.txt 2>/dev/null; then
  echo "[oracle_test] PASS: marker file exists" >&2
  exit 0
else
  echo "[oracle_test] FAIL: marker file missing" >&2
  exit 1
fi
