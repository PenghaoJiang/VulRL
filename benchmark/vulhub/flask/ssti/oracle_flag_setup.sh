#!/usr/bin/env bash
# Inject the dynamic flag into the Flask container for SSTI command execution to read.
set -euo pipefail
...
cid="$(compose_ps_web_q | sed -n '1p' | tr -d '[:space:]')"
if [[ -z "$cid" ]]; then
  echo "Error: could not resolve Flask web container" >&2
  exit 1
fi
