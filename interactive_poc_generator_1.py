...
def _build_agent1_system_prompt(target_host: str, target_port: int) -> str:
...
3. Submit a verified PoC script

You have access to these tools:
- bash: Execute bash commands in an attacker container on the same Docker network as the target
...
5. Execute the PoC script to verify the exploit effect (within attacker container)
6. If the exploit effect is not as expected, repeat the process from step 3
7. Submit the PoC script with submit_poc
...
- Be methodical: probe first, then exploit
...
class InteractivePoCPipelineV2:
    ...
    def process_cve(self, cve_dir: Path, scanner: VulhubScanner) -> Optional[PoCBundle]:
        ...
        try:
            ...
            poc_args = f"--host {docker_env.target_host} --port {docker_env.target_port}"
