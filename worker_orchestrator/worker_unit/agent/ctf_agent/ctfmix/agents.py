...
from .commands import Command, ParseCommand
from .history_processors import HistoryProcessor
from .models import (
    APIStats,
...
if TYPE_CHECKING:
    from .runtime import CTFMixRuntime as SWEEnv
else:
    SWEEnv = Any
...
class AgentConfig(FrozenSerializable):
    ...
    state_command: Command = Command(
        name="state",
        code="""state() {
            local working_dir="$(realpath --relative-to=$ROOT/.. $(pwd -P))";
            local open_file="n/a";
    ...
    def init_environment_vars(self, env: SWEEnv):
        self.set_environment_vars(env, self.env_variables)

    def set_environment_vars(self, env: SWEEnv, env_variables: dict[str, Any]) -> None:
        commands_to_execute = [self.state_command.code] + [f"{k}={v}" for k, v in env_variables.items()]
        output = env.communicate("\n".join(commands_to_execute))
        if env.returncode != 0:
            raise RuntimeError(f"Failed to set environment variables: {output}")
        ...
        for file in self.command_files:
            ...
            command_names = [command.name for command in parsed_commands if command.name]

            datum["source_name"] = filename
            if filename.endswith(".sh"):
                datum["name"] = filename
                datum["type"] = "source_file"
            elif command_names:
                datum["name"] = command_names[0]
                datum["type"] = "script"
            elif filename.startswith("_"):
                datum["name"] = Path(filename).stem
                datum["type"] = "utility"
            else:
                msg = (
                    f"Could not determine how to install command file {file}.\n"
                    "Expected either a sourced shell file, a documented executable command, or a helper utility prefixed with underscore."
                )
                raise ValueError(msg)
...
class Agent:
    ...
    def traj_path(self) -> Path | None:
        ...
        if self.traj_dir and self._env is not None:
            ...
            # Ensure traj_dir is a Path object (could be string from caller)
            traj_dir_path = Path(self.traj_dir) if isinstance(self.traj_dir, str) else self.traj_dir
            return traj_dir_path / (self._env.record["instance_id"] + ".traj")
        return None
    ...
    def save_trajectory(
        ...
        # Only save if traj_path is set (i.e., traj_dir was provided)
    ...
    def forward_model(self, observation: str | None, state: str) -> str:
        ...
        try:
            ...
            if not state or not state.strip():
                ...
                state_vars = {"open_file": "n/a", "working_dir": ".", "interactive_session": "n/a"}
            else:
        ...
        state_vars.setdefault("open_file", "n/a")
        state_vars.setdefault("working_dir", ".")
        state_vars.setdefault("interactive_session", "n/a")
    ...
    def set_environment_vars(self, env: SWEEnv, env_variables: dict[str, Any]) -> None:
        ...
        for file in self.config.command_files:
            ...
            datum["contents"] = contents
            filename = Path(file).name
            parsed_commands = self.config.parse_command.parse_command_file(file)
            command_names = [command.name for command in parsed_commands if command.name]
    ...
    def run(
        ...
        if self.traj_path:
            ...
            self.logger.info("Trajectory saving disabled (traj_dir=None)")
        ...
        while not done:
            if self.model.args.per_instance_step_limit > 0 and len(self.trajectory) >= self.model.args.per_instance_step_limit:
                self.logger.warning(
                    f"Step limit reached after {len(self.trajectory)} steps "
                    f"(limit: {self.model.args.per_instance_step_limit})"
                )
                self.info["exit_status"] = "step_limit"
                self.info["submission"] = None
                break
