...
class ParseCommandBash(ParseCommand):
    ...
    def parse_bash_functions(self, path, contents: str) -> list[Command]:
        ...
        while idx < len(lines):
            ...
            if line.startswith("# @yaml"):
                docs = ["@yaml"]
            elif docs is not None and line.startswith("#"):
                docs.append(line[2:] if line.startswith("# ") else line[1:])
            elif docs is not None and line.strip() == "":
                continue
                ...
                docs_dict = yaml.safe_load("\n".join(docs).replace("@yaml", "")) if docs is not None else None
    ...
    def parse_script(self, path, contents) -> list[Command]:
        ...
        if len(matches) == 0:
            ...
            name = signature.split()[0] if signature else Path(path).name.rsplit(".", 1)[0]
